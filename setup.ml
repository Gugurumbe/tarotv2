...
(* OASIS_START *)
(* DO NOT EDIT (digest: 27a68f5f4803bb94cb3cd49f36d51055) *)
(*
...
let setup_t =
  {
            ...
            [
               Library
                 ({
                   ...
                   {
                      lib_modules = ["Value"; "Lwt_value"];
                      lib_pack = false;
                      lib_internal_modules = ["Value_read"; "Comparaison"];
               ...
               Library
                 ({
                  ...
                  },
                   {
                      bs_build = [(OASISExpr.EBool true, true)];
                      bs_install = [(OASISExpr.EBool true, true)];
                      bs_path = "src/libtarotv_server";
                      bs_compiled_object = Best;
                      bs_build_depends =
                        [
                           InternalLibrary "libtarotv_protocol";
                 ...
                 ({
                  ...
                  },
                   {
                      ...
                      bs_build_depends =
                        [
                           FindlibPackage ("lwt", None);
                           InternalLibrary "libtarotv_server";
                           InternalLibrary "libtarotv_protocol"
                        ];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = [];
               ...
               Executable
                 ({
                  ...
                  },
                   {
                      bs_build = [(OASISExpr.EBool true, true)];
                      bs_install = [(OASISExpr.EBool true, true)];
                      bs_path = "src/calculator";
                      bs_compiled_object = Best;
                      bs_build_depends =
                        [
                           InternalLibrary "libtarotv_server";
                           InternalLibrary "libcalculator";
                           InternalLibrary "libtarotv_protocol"
                        ];
                      bs_build_tools = [ExternalTool "ocamlbuild"];
                      bs_c_sources = [];
               ...
               Executable
                 ({
                   ...
                   {
                      exec_custom = false;
                      exec_main_is = "client_qt.cxxnative.ml"
                   })
            ];
          plugins =
            [
     ...
     oasis_version = "0.4.5";
     oasis_digest = Some "#\190)\026n\169\000\210\141\242\004*x\158c]";
     oasis_exec = None;
     oasis_setup_args = [];
     setup_update = false
  };;
...
let standard_paths =
  let path_sep =
    ...
    | _ ->
      ':'
  in
  OASISString.nsplit (Sys.getenv "PATH") path_sep

let which pathlst fname =
  let exec_ext =
    match Sys.os_type with
    | "Win32" ->
      "" :: (OASISString.nsplit (Sys.getenv "PATHEXT") ';')
    | _ ->
      [""]
  in
...
let setup_cmake () =
  ...
  then
    let cmake_exec = which standard_paths "cmake" in
    let make_exec = which standard_paths "make" in
    let () = Printf.printf "CMake configuration: \n" in
    let chan = open_out_bin "setup.cmake.data" in
